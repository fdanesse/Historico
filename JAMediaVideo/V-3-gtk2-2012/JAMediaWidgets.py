...
class Barra_de_Reproduccion(gtk.HBox):
    ...
    def __init__(self):
        ...
        pixbuf = gtk.gdk.pixbuf_new_from_file(os.path.join(G.ICONOS, "siguiente.png")).flip(True)
...
class JAMediaList(gtk.EventBox):
    ...
    def select_item(self):
        ...
        self.elementselected.eventbox.modify_bg(gtk.STATE_NORMAL,
            self.elementselected.colornormal)
        
        if "image" in self.elementselected.mimetype:
            self.emit("loadimagen", self.elementselected.archivo)
            
        elif "video" in self.elementselected.mimetype or \
            "audio" in self.elementselected.mimetype:
...
class ElementoEnLista(gtk.Fixed):
    ...
    def __init__(self, texto, archivo):
        ...
        self.mimetype = G.describe_archivo(self.archivo)
        ...
        elif "video" in self.mimetype or "application/ogg" in self.mimetype :
            self.mimetype = "video"
...
class VentanaConfiguracionfilmar(gtk.Window):
    ...
    def __init__(self, ventanajamedia):
        ...
        import platform
        if not 'olpc' in platform.platform():
        ...
        self.comboaudiorate.set_active(0)
        ...
        self.comboaudiodepth.append_text("8")
