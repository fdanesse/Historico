...
class JAMediaVideo(activity.Activity):
    ...
    # SEÑALES >>
    def set_config_filmar(self, widget, device,
        resolucion, audiorate, audiochannels, audiodepth):
            
        w, h = resolucion.split("x")
        w, h = (int(w), int(h))
        self.webcam.set_config( device = device, resolucion = (w,h),
            audiorate = int(audiorate), audiochannels = int(audiochannels),
            audiodepth = int(audiodepth) )

    def set_config_grabar_audio(self, widget, audiorate,
        audiochannels, audiodepth):
        self.webcam.set_config_audio( audiorate = int(audiorate),
            audiochannels = int(audiochannels), audiodepth = int(audiodepth) )
    ...
    def get_menu(self, boton, pos, tiempo, widget):
        ...
        audio = gtk.MenuItem("Cargar Archivos de JAMedia")
        menu.append(audio)
        audio.connect_object("activate", self.load_list, "JAMedia")

        audio = gtk.MenuItem("Cargar Archivos de JAMediaTube")
        menu.append(audio)
        audio.connect_object("activate", self.load_list, "JAMediaTube")
    ...
    def load_list(self, tipo):
        ...
        if tipo == "Videos":
            ...
            archivos = os.listdir(G.DIRECTORIOJAMEDIA)
            lista= []
            for archivo in archivos:
                lista.append([archivo, os.path.join(G.DIRECTORIOJAMEDIA, archivo)])
            self.jamedialist.set_list(lista)
            self.widgetvideo.expose()
            
        elif tipo == "JAMediaTube":
            archivos = os.listdir(G.DIRECTORIOJAMEDIATUBE)
            lista= []
            for archivo in archivos:
                lista.append([archivo, os.path.join(G.DIRECTORIOJAMEDIATUBE, archivo)])
            self.jamedialist.set_list(lista)
            self.widgetvideo.expose()
    ...
    def ver_controles(self, widget = None, valor = None):
        ...
        if self.menuactual != self.menureproduccion and \
            self.menuactual != self.menuverimagenes: return
    ...
    def backmenu(self, widget = None, senial = None):
        ...
        if self.menuactual == self.menufilmar and \
            self.webcam.estado == "Grabando":
            self.webcam.stop()
            
        if self.menuactual == self.menugrabaraudio and \
            self.webcam.estado == "Grabando":
            self.webcam.stopaudio()
