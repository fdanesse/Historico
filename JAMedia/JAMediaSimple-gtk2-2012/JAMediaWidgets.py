...
class Superficie_de_Reproduccion(gtk.DrawingArea):
    ...
    def repaintimagen(self, widget=None, event=None):
        ...
        if self.imagen:
            ...
            scaledPixbuf = self.imagen.scale_simple(
                ww, hh, gtk.gdk.INTERP_BILINEAR)
...
class Barra_de_Progreso(gtk.EventBox):
    ...
    def changevalueprogressbar(self, widget=None, flags=None, valor=None):
        ...
        if valor < 0 or valor > self.scale.ajuste.upper - 2:
            return
...
class ToolbarLista(gtk.Toolbar):
    ...
    def __init__(self):
        ...
        boton.show()
        tooltips = gtk.Tooltips()
        tooltips.set_tip(boton, _("Selecciona una Lista."),
            tip_private=None)
        ...
        tooltips = gtk.Tooltips()
        tooltips.set_tip(self.boton_agregar, _("Agregar Streaming"),
            tip_private=None)
