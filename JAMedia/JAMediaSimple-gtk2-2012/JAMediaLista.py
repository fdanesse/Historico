...
class JAMediaLista (gtk.ScrolledWindow):
    ...
    def next(self):
        ...
        if not self.contenedor.get_children():
            return
...
class JAMediaItem(gtk.EventBox):
    ...
    __gsignals__ = {
    "click": (gobject.SIGNAL_RUN_FIRST,
        gobject.TYPE_NONE, []),
    ...
    def __init__(self, preview, texto, url, tipo):
        ...
        self.add_events(
            gtk.gdk.BUTTON_PRESS_MASK | gtk.gdk._2BUTTON_PRESS |
        ...
        if not preview or not os.path.exists(preview):
            preview = os.path.join(G.ICONOS, "fondo.png")
