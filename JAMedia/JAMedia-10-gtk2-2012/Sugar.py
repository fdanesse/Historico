...
class JAMedia(activity.Activity):
    ...
    def read_file(self, file_path):
        ...
        self.toolbar_list.label.set_text(_("File from Journal"))
    ...
    def paint_pantalla(self, widget, event):
        ...
        if not self.videoenfuente or \
            self.item_activo_en_lista.tipo == "Radio" or \
            self.mplayer_server.get_estado() == "stoped Audio_Video":
    ...
    def download_streamings(self, widget, datos):
        ...
        self.Lista_de_Television = self.origenes_de_datos.Cargar_Tabla(
            "Television")
        dialog = StandarDialog(self, _("JAMedia Message"), _(mensaje))
        dialog.run()
        dialog.destroy()
    ...
    def load_and_play(self, widget, item):
        ...
        self.mplayer_server.play(
            self.item_activo_en_lista.url,
    ...
    def set_menu_list(self, widget, item, boton, pos, tiempo):
        ...
        if item.tipo == "Audio_Video":
            ...
            quitar.connect_object("activate",
                self.set_accion_archivos, item, "Quitar")
            ...
            if self.toolbar_list.label.get_text() == _("JAMedia Radio")\
                ...
                menu.append(copiar)
                copiar.connect_object("activate",
                    self.set_accion_stream, item, "Copiar")
                mover = gtk.MenuItem(_("Move to my stream"))
                menu.append(mover)
                mover.connect_object("activate",
                    self.set_accion_stream, item, "Mover")

            borrar = gtk.MenuItem(_("Delete stream"))
            menu.append(borrar)
            borrar.connect_object("activate",
                self.set_accion_stream, item, "Borrar")
            grabar = gtk.MenuItem(_("Record transmission"))
            menu.append(grabar)
            grabar.connect_object("activate",
                self.set_accion_stream, item, "Grabar")
    ...
    def set_accion_stream(self, item, accion):
        ...
        if accion == "Quitar":
            ...
            dialog = StandarDialog(self, _("JAMedia Message"), _("Are you sure you want to move this stream to your list?"))
            response = dialog.run()
            dialog.destroy()
    ...
    def add_stream(self, widget):
        ...
        dialog = gtk.Dialog(title=_("JAMedia Message"),
            parent=self,
    ...
    def new_streaming(self, widget, texto1, texto2):
        ...
        if not nombre or not direccion:
            dialog = StandarDialog(self,
                _("JAMedia Message"),
                _("You must specify a name and a playing URL."))
            response = dialog.run()
            dialog.destroy()
            return
    ...
    def grabar_streaming(self, item):
        ...
        archivo = "%s-%s-%s%s" % (
            fecha, hora, item.texto.split("(")[0], extension)
        ...
        dialog = StandarDialog(
            self, _("JAMedia Message"), _("Start to record?"))
    ...
    def comenzar_grabacion(self, archivo, direccion):
        ...
        ejecutable = os.path.join(
            G.DIRECTORIO_BASE, "Mplayer_Grabador.py")

        if self.graba:
            self.detener_grabacion()

        self.graba = subprocess.Popen(
            '%s****%s' % (
            direccion, archivo),
    ...
    def dialog_salir(self, widget=None, valor=None):
        ...
        dialog = StandarDialog(self,
            "JAMedia Message", _("Exit from JAMedia?"))
        response = dialog.run()
        dialog.destroy()
