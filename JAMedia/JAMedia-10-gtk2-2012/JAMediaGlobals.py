...
if os.path.exists(directorio_viejo):
    for elemento in os.listdir(directorio_viejo):
        commands.getoutput('mv %s %s' % (
            os.path.join(directorio_viejo, elemento),
            directorio_nuevo))
...
DIRECTORIO_DATOS = os.path.join(
    os.environ["HOME"], "JAMediaDatos", "Datos")
...
DIRECTORIO_YOUTUBE = os.path.join(
    os.environ["HOME"], "JAMediaDatos", "YoutubeVideos")
...
AUDIO_JAMEDIA_VIDEO = os.path.join(
    os.environ["HOME"], "JAMediaDatos", "Audio")
...
VIDEO_JAMEDIA_VIDEO = os.path.join(
    os.environ["HOME"], "JAMediaDatos", "Videos")
...
# Versiones viejas de gtk no funcionan si no se usa 0 a 65000.
# Ejem: 122*65000/255= 26000
