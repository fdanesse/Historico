...
class JAMediaMixer(gtk.Window):
    ...
    def reset_sound(self, widget=None):
        ...
        for control in self.mixer_controls:
            if "Speaker" in control or \
                "Master" in control or "PCM" in control:
...
class Control(gtk.Frame):
    ...
    def __init__(self, control):
        ...
        self.scale = VolumenControl(
            gtk.Adjustment(0.0, 1.0, 101.0, 0.1, 1.0, 1.0))
        self.get_volumen()
    ...
    def get_volumen(self, widget=None):
        self.scale.set_value(
            get_control_volumen(control=self.control))
...
class VolumenControl(gtk.VScale):
    ...
    def expose(self, widget, event):
        ...
        yy = int((h - altoimagen) * (
            self.ajuste.upper - self.get_value()) / (
            self.ajuste.upper - self.ajuste.lower))
        ...
        self.window.draw_rectangle(
            gc, True, x + w / 2 - ancho / 2,
            y + borde, ancho, h - borde * 2)
        ...
        if altura > 0:
            self.window.draw_rectangle(
                gc, True, x + w / 2 - ancho / 2,
                y + yimagen - borde, ancho, altura)

        gc.set_rgb_fg_color(G.NEGRO)
        self.window.draw_rectangle(
            gc, False, x + w / 2 - ancho / 2,
            y + borde, ancho, h - borde * 2)
