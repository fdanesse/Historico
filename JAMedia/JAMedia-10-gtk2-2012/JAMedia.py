...
from JAMediaWidgets import ButtonJAMediaMixer
from JAMediaWidgets import Barra_de_Progreso
from JAMediaWidgets import Barra_de_Opciones_uno
from JAMediaWidgets import Barra_de_Reproduccion
from JAMediaWidgets import JAMediaInfo
from JAMediaWidgets import StandarDialog
from JAMediaWidgets import ToolbarLista
from JAMediaWidgets import ControlGrabar
from JAMediaWidgets import Superficie_de_Reproduccion
from JAMediaWidgets import Selector_de_Archivos
...
class JAMedia(gtk.Window):
    ...
    def __init__(self):
        ...
        self.origenes_de_datos = Archivos_y_Directorios(
            G.DIRECTORIO_DATOS, G.DIRECTORIO_MIS_ARCHIVOS)
        self.origenes_de_datos.Generar_Origenes_de_Datos()
        self.Lista_de_Radios = self.origenes_de_datos.Cargar_Tabla(
            "Radios")
        self.Lista_de_Television = self.origenes_de_datos.Cargar_Tabla(
            "Television")
        self.Lista_de_Archivos = None
        self.Lista_de_MisRadios = self.origenes_de_datos.Cargar_Tabla(
            "MisRadios")
        self.Lista_de_MisTelevision = self.origenes_de_datos.Cargar_Tabla(
            "MisTelevision")
        ...
        self.info_grabar = ControlGrabar(_("Stopped recording."))
        self.info_reproduccion = JAMediaInfo(_("Stopped playing."))
        ...
        self.barradeopcionesuno.connect("set_video_resolucion",
            self.set_video_resolucion)
        self.barradeopcionesuno.connect("download_streamings",
            self.download_streamings)
        ...
        self.barradeprogreso.connect("change-value",
            self.changevalueprogressbar)
    ...
    def mplayer_info(self, widget=None, valor=None):
        if self.info_reproduccion.label.get_text() != valor:
            self.info_reproduccion.label.set_text(valor)

    def changevalueprogressbar(self, widget=None, valor=None):
        estados = ["playing Audio_Video",
            "paused Audio_Video", "stoped Audio_Video"]

        if self.mplayer_server.get_estado() in estados:
            self.mplayer_server.seek(valor)

    def paint_pantalla(self, widget, event):

        if not self.videoenfuente or \
            self.item_activo_en_lista.tipo == "Radio" \
    ...
    def cambio_estado(self, widget, valor):
        """
        "playing Audio_Video", "paused Audio_Video",
        "stoped Audio_Video", "playing Radio", "playing TV", None
        """

        if valor == "playing Audio_Video" or \
            valor == "playing Radio" or valor == "playing TV":
            self.barradereproduccion.set_playing()

        elif valor == "paused Audio_Video" or \
            valor == "stoped Audio_Video" or valor == None:
    ...
    def load_and_play(self, widget, item):
        ...
        if ISOLPC:
            self.videoenfuente = True  # por compatibilidad con sugar

        self.mplayer_server.play(self.item_activo_en_lista.url,
            self.item_activo_en_lista.tipo)
    ...
    def activar(self, widget=None, senial=None):
        ...
        if senial == "stop":
            ...
            if active_item:
                if self.mplayer_server.get_estado() == "stoped Audio_Video" or \
                    self.mplayer_server.get_estado() == None:
                ...
                elif self.mplayer_server.get_estado() in [
                    "paused Audio_Video", "playing Audio_Video"]:
    ...
    def load_list(self, widget, indice):
        ...
        # El parámetro None, se agregó para los
        # previews de la lista (JAMedia-10),
        ...
        # llenar lista con Streaming del usuario
            ...
            self.lista_de_reproduccion.set_lista(lista)
            #TRANS: Radio Stations
            self.toolbar_list.label.set_text(_("My Stations"))
        ...
        # llenar lista con Streaming del usuario
            ...
            self.lista_de_reproduccion.set_lista(lista)
            #TRANS: TV Channels
            self.toolbar_list.label.set_text(_("My Channels"))
        ...
        # llenar lista con archivos de videos en el directorio de jamedia
            ...
            self.lista_de_reproduccion.set_lista(lista)
            self.toolbar_list.label.set_text(_("My Files"))
    ...
    # crea una lista de archivos y la selecciona para reproduccion.
        if self.Lista_de_Archivos:
            ...
            self.lista_de_reproduccion.set_lista(lista)
            self.toolbar_list.label.set_text(_("External Files"))

    def set_menu_list(self, widget, item, boton, pos, tiempo):
        menu = gtk.Menu()

        if item.tipo == "Audio_Video":
            quitar = gtk.MenuItem(_("Remove from the list"))
            menu.append(quitar)
            quitar.connect_object("activate",
                self.set_accion_archivos, item, "Remove")
            ...
            if Plectura and origen != destino:
                copiar = gtk.MenuItem(_("Copy to JAMedia"))
                menu.append(copiar)
                copiar.connect_object("activate",
                    self.set_accion_archivos, item, "Copy")

            if Pescritura and origen != destino:
                mover = gtk.MenuItem(_("Move to JAMedia"))
                menu.append(mover)
                mover.connect_object("activate",
                    self.set_accion_archivos, item, "Move")

            if Pescritura:
                borrar = gtk.MenuItem(_("Delete this file"))
                menu.append(borrar)
                borrar.connect_object("activate",
                    self.set_accion_archivos, item, "Delete")

        else:
            quitar = gtk.MenuItem(_("Remove from the list"))
            menu.append(quitar)
            quitar.connect_object("activate",
                self.set_accion_stream, item, "Remove")

            if self.toolbar_list.label.get_text() == _("JAMedia Radio")\
                or self.toolbar_list.label.get_text() == _("JAMedia TV"):
                copiar = gtk.MenuItem(_("Copy to my stream"))
                menu.append(copiar)
                copiar.connect_object("activate",
                    self.set_accion_stream, item, "Copy")
                mover = gtk.MenuItem(_("Move to my stream"))
                menu.append(mover)
                mover.connect_object("activate",
                    self.set_accion_stream, item, "Move")

            borrar = gtk.MenuItem(_("Remove stream"))
            menu.append(borrar)
            borrar.connect_object("activate",
                self.set_accion_stream, item, "Delete")
            grabar = gtk.MenuItem(_("Record transmission"))
            menu.append(grabar)
            grabar.connect_object("activate",
                self.set_accion_stream, item, "Record")
    ...
    def set_accion_stream(self, item, accion):
        ...
        if accion == "Remove":
            dialog = StandarDialog(self, _("JAMedia Message"),
                _("Are you sure you want to remove it from the list?"))
            response = dialog.run()
            dialog.destroy()
            ...
            dialog = StandarDialog(self, _("JAMedia Message"),
                _("Are you sure you want to remove the stream?"))
            response = dialog.run()
            dialog.destroy()
            ...
            # borrar streaming del archivo segun self.tipo
                ...
                if self.toolbar_list.label.get_text() == _("JAMedia Radio"):
                    self.origenes_de_datos.eliminar_streaming(
                        stream, "jamediaradio")
                    self.Lista_de_Radios = self.origenes_de_datos.Cargar_Tabla(
                        "Radios")
                ...
                elif self.toolbar_list.label.get_text() == _("My Stations"):
                    self.origenes_de_datos.eliminar_streaming(
                        stream, "MisRadios")
                    self.Lista_de_MisRadios = self.origenes_de_datos.Cargar_Tabla("MisRadios")

                elif self.toolbar_list.label.get_text() == _("My Channels"):
                    self.origenes_de_datos.eliminar_streaming(
                        stream, "MisTelevision")
                    self.Lista_de_MisTelevision = self.origenes_de_datos.Cargar_Tabla("MisTelevision")

        elif accion == "Copiar":
            dialog = StandarDialog(self, _("JAMedia Message"),
                _("Are you sure you want to copy this stream to your list?"))
            response = dialog.run()
            dialog.destroy()

            if response == gtk.RESPONSE_YES:
                if self.toolbar_list.label.get_text() == _("JAMedia Radio"):
                    self.origenes_de_datos.agregar_streaming(
                        stream, self.origenes_de_datos.misradios)
                    self.Lista_de_MisRadios = self.origenes_de_datos.Cargar_Tabla("MisRadios")

                elif self.toolbar_list.label.get_text() == _("JAMedia TV"):
                    self.origenes_de_datos.agregar_streaming(
                        stream, self.origenes_de_datos.mistv)
                    self.Lista_de_MisTelevision = self.origenes_de_datos.Cargar_Tabla("MisTelevision")

        elif accion == "Mover":
            dialog = StandarDialog(self, _("JAMedia Message"),
            _("Are you sure you want to move this stream to your list?"))
            response = dialog.run()
            dialog.destroy()
    ...
    def add_stream(self, widget):
        ...
        #boton_OK = gtk.Button(_("Yes"))
        #boton_Cancel = gtk.Button(_("No"))

        dialog = gtk.Dialog(
            title="JAMedia Message",
            parent=self,
        ...
        #Botones
        caja3 = gtk.HBox()
        boton_guardar = gtk.Button(_("Save"))
        boton_cancelar = gtk.Button(_("Cancel"))
    ...
    def new_streaming(self, widget, texto1, texto2):
        ...
        if not nombre or not direccion:
            dialog = StandarDialog(self, "JAMedia Message",
                _("You must specify a name and a URL to play."))
            response = dialog.run()
            dialog.destroy()
            return
        ...
        dialog = StandarDialog(self, "JAMedia Message",
            _("Are you sure you want to add this stream to your list?"))
        response = dialog.run()
        dialog.destroy()

        if response == gtk.RESPONSE_YES:
            if self.toolbar_list.label.get_text() == _("My Stations"):
                self.origenes_de_datos.agregar_streaming(
                    stream, self.origenes_de_datos.misradios)
                self.Lista_de_MisRadios = self.origenes_de_datos.Cargar_Tabla("MisRadios")
    ...
    def set_accion_archivos(self, item, accion):
        ...
        if accion == "Copiar":
            dialog = StandarDialog(self, "JAMedia Message",
                _("Are you sure you want to copy this file?"))
            response = dialog.run()
            dialog.destroy()
            ...
            dialog = StandarDialog(self, "JAMedia Message",
                _("Are you sure you want to move this file?"))
            response = dialog.run()
            dialog.destroy()
    ...
    def grabar_streaming(self, item):
        ...
        archivo = "%s-%s-%s%s" % (fecha, hora,
            item.texto.split("(")[0], extension)
        ...
        dialog = StandarDialog(self, "JAMedia Message", _("Start to record?"))
        response = dialog.run()
        dialog.destroy()
    ...
    def comenzar_grabacion(self, archivo, direccion):
        ...
        self.graba = subprocess.Popen('%s****%s' % (direccion, archivo),
            executable=ejecutable, shell=True)
    ...
    def dialog_salir(self, widget=None, valor=None):
        dialog = StandarDialog(
            self, _("JAMedia Message"), _("Exit from JAMedia?"))
        response = dialog.run()
        dialog.destroy()
