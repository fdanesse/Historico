...
class Archivos_y_Directorios():
    ...
    def __init__(self, DIRECTORIO_DATOS, DIRECTORIO_MIS_ARCHIVOS):
        ...
        self.jamediatv = os.path.join(
            self.directorio_base, "jamediatv.txt")
        self.jamediaradio = os.path.join(
            self.directorio_base, "jamediaradio.txt")
        self.misradios = os.path.join(
            self.directorio_base, "misradios.txt")
        self.mistv = os.path.join(
            self.directorio_base, "mistv.txt")
    ...
    def Generar_Origenes_de_Datos(self):
        ...
        if not os.path.exists(self.jamediatv):
            ...
            archivo.close()
            os.chmod(os.path.join(self.directorio_base,
                "jamediatv.txt"), 0666)
        ...
        if not lista:
            self.Actualizar_Streaming_de_JAMedia()
    ...
    def Actualizar_Streaming_de_JAMedia(self):
        ...
        try:
            ...
            os.chmod(os.path.join(self.directorio_base,
                "jamediaradio.txt"), 0666)
            return _("The streams were successfully updated.")

        except:
            return _("The streams could not be updated.")
        ...
        try:
            if os.path.exists(direccion):
                ...
                if not os.path.basename(direccion) in archivos:
                    ...
                    dialog.set_title(_("JAMedia Message"))
                    dialog.set_markup(_("The file already exists in the destination directory. Do you want to overwrite it?"))
                    response = dialog.run()
                    dialog.destroy()
    ...
    def get_mensaje_copiado(self):
        ...
        dialog.set_title(_("JAMedia Message"))
        dialog.set_markup(_("File copied."))
        dialog.run()
        dialog.destroy()
        ...
        try:
            if os.path.exists(direccion):
                ...
                dialog = gtk.MessageDialog(None,
                    gtk.DIALOG_MODAL, gtk.MESSAGE_INFO, gtk.BUTTONS_OK)
                dialog.set_title(_("JAMedia Message"))
                dialog.set_markup(_("The file does not exist."))
                dialog.run()
                dialog.destroy()
    ...
    def get_mensaje_borrado(self):
        dialog = gtk.MessageDialog(None,
            gtk.DIALOG_MODAL, gtk.MESSAGE_INFO, gtk.BUTTONS_OK)
        dialog.set_title(_("JAMedia Message"))
        dialog.set_markup(_("File deleted."))
        dialog.run()
        dialog.destroy()
    ...
    def get_mensaje_movido(self):
        dialog = gtk.MessageDialog(None,
            gtk.DIALOG_MODAL, gtk.MESSAGE_INFO, gtk.BUTTONS_OK)
        dialog.set_title(_("JAMedia Message"))
        dialog.set_markup(_("File moved."))
        dialog.run()
        dialog.destroy()
