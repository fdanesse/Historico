...
class Mplayer_Reproductor(gobject.GObject):
    ...
    __gsignals__ = {
    "cambio_estado": (gobject.SIGNAL_RUN_FIRST,
        gobject.TYPE_NONE, (gobject.TYPE_STRING,)),
    "update_progress": (gobject.SIGNAL_RUN_FIRST,
        gobject.TYPE_NONE, (gobject.TYPE_FLOAT,)),
    "video": (gobject.SIGNAL_RUN_FIRST,
        gobject.TYPE_NONE, (gobject.TYPE_BOOLEAN,)),
    "mplayer_info": (gobject.SIGNAL_RUN_FIRST,
        gobject.TYPE_NONE, (gobject.TYPE_STRING,))}

    def __init__(self, id_pantalla):
        """
        Posibles estados: "playing Audio_Video",
        "paused Audio_Video", "stoped Audio_Video",
        "playing Radio", "playing TV", None
        """
        ...
        """
        estructura = "%s -wid %i -slave -idle -nolirc -quiet −volume 100" % (
            self.ejecutable, self.id_pantalla)
        self.mplayer = subprocess.Popen(
            estructura, shell=True, stdin=subprocess.PIPE,
            stdout=open(STDOUT,"w+b"), stderr=open(STDERR,"r+b"),
    ...
    def update_progress(self):
        ...
        if not self.entrada.closed:
            ...
            if linea:
                if "ANS_PERCENT_POSITION" in linea:
                ...
                elif "Position:" in linea or \
                    "VO:" in linea or "AO:" in linea:
                    ...
                    "AUDIO" in linea or "Selected" in linea or \
                    "Genre" in linea or "Website" in linea or \
                    "Bitrate" in linea:
                    """
    ...
    def get_progress_cache_in_mplayer(self, linea):
        ...
        if "Cache not responding" in linea:
            return
    ...
    def seek(self, valor):
        ...
        self.Actualizador = gobject.timeout_add(
            UPDATE_TIME, self.update_progress)
        ...
        try:
            if self.entrada:
                if self.get_estado() == "playing Audio_Video":  # pausa
                    ...
                    self.emit("mplayer_info", _("Playback Paused"))
                    ...
                    self.Actualizador = gobject.timeout_add(
                        UPDATE_TIME, self.update_progress)
                    self.set_estado("playing Audio_Video")
                    self.emit("mplayer_info", "%s: %s" % (
                        _("Playing"), self.pista_actual))
    ...
    def quit(self, widget=None):
        ...
        self.pista_actual = None
        self.set_estado(None)
        self.emit("mplayer_info", _("Playback Stopped"))
