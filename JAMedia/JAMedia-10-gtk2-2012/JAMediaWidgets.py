...
class Superficie_de_Reproduccion(gtk.DrawingArea):
    ...
    def __init__(self):
        ...
        self.img = cairo.ImageSurface.create_from_png(
            os.path.join(G.ICONOS, "fondo.png"))
...
class Barra_de_Opciones_uno(gtk.HBox):
    ...
    __gsignals__ = {
    "set_video_resolucion": (gobject.SIGNAL_RUN_FIRST,
        gobject.TYPE_NONE, (gobject.TYPE_INT, gobject.TYPE_INT)),
    "salir": (gobject.SIGNAL_RUN_FIRST,
        gobject.TYPE_NONE, (gobject.TYPE_BOOLEAN,)),
    "download_streamings": (gobject.SIGNAL_RUN_FIRST,
    ...
    def __init__(self, ventanajamedia):
        ...
        self.ventanadescargas.connect(
            "download_streamings", self.download_streamings)
        self.ventanadescargas.hide()

        self.salir = JAMediaButton()
        self.salir.set_tooltip(_("Exit"))
        ...
        self.descarga.set_tooltip(_("Downloads"))
        pixbuf = gtk.gdk.pixbuf_new_from_file(
            os.path.join(G.ICONOS, "iconplay.png"))
        self.descarga.imagen.set_from_pixbuf(
            pixbuf.rotate_simple(gtk.gdk.PIXBUF_ROTATE_CLOCKWISE))
        self.descarga.set_tamanio(lado, lado)
        self.descarga.connect("clicked", self.get_VentanaDescargas)

        self.escalar = JAMediaButton()
        self.escalar.set_tooltip(_("Resolution"))
    ...
    def menu(self, widget, boton, pos, tiempo):
        menu = gtk.Menu()
        menu.set_title(_("Scale video to:"))
        ...
        for res in resoluciones[0:-1]:
            if res[0] < ancho and res[1] < alto:
                ...
                menu.append(item)
                item.connect_object(
                    "activate", self.set_video_resolucion, res)

        item = gtk.MenuItem(_("Fullscreen"))
        menu.append(item)
        item.connect_object(
            "activate", self.set_video_resolucion, "fullscreen")
...
class Selector_de_Archivos (gtk.FileChooserDialog):
    ...
    def __init__(self, jamedia):
        ...
        gtk.FileChooserDialog.__init__(
            self, title=_("Open Audio or Video Files"),
            parent=self.jamedia,
        ...
        # extras
        hbox = gtk.HBox()
        boton_abrir_directorio = gtk.Button(_("Open"))
        boton_seleccionar_todo = gtk.Button(_("Select all"))
        boton_salir = gtk.Button(_("Exit"))
        ...
        boton_seleccionar_todo.connect(
            "clicked", self.seleccionar_todos_los_archivos)
...
class Barra_de_Progreso(gtk.Frame):
    ...
    __gsignals__ = {
    "change-value": (gobject.SIGNAL_RUN_FIRST,
...
class ProgressBar(gtk.HScale):
    ...
    def __init__(self, ajuste):
        ...
        pixbuf = gtk.gdk.pixbuf_new_from_file_at_size(
            os.path.join(G.ICONOS, "iconplay.png"), 25, 25)
    ...
    def expose(self, widget, event):
        ...
        self.window.draw_pixbuf(
            gc, self.pixbuf, 0, 0, ximagen, yimagen,
...
class Barra_de_Reproduccion(gtk.HBox):
    ...
    def __init__(self):
        ...
        self.botonatras.set_tooltip(_("Last track"))
        pixbuf = gtk.gdk.pixbuf_new_from_file_at_size(
            os.path.join(G.ICONOS, "siguiente.png"), 32, 32).flip(True)
        ...
        self.botonplay.set_tooltip(_("Play or pause"))
        pixbuf = gtk.gdk.pixbuf_new_from_file_at_size(
            os.path.join(G.ICONOS, "play.png"), 32, 32)
        ...
        self.botonsiguiente.set_tooltip(_("Next track"))
        pixbuf = gtk.gdk.pixbuf_new_from_file_at_size(
            os.path.join(G.ICONOS, "siguiente.png"), 32, 32)
        ...
        self.botonstop.set_tooltip(_("Stop playing"))
        pixbuf = gtk.gdk.pixbuf_new_from_file_at_size(
            os.path.join(G.ICONOS, "stop.png"), 32, 32)
    ...
    def set_playing(self):
        ...
        pixbuf = gtk.gdk.pixbuf_new_from_file_at_size(
            os.path.join(G.ICONOS, "pausa.png"), 32, 32)
        self.botonplay.imagen.set_from_pixbuf(pixbuf)
...
class ButtonJAMediaMixer(JAMediaButton):
    ...
    def __init__(self):
        ...
        pixbuf = gtk.gdk.pixbuf_new_from_file_at_size(
            os.path.join(G.ICONOS, "volumen.png"), 32, 32)
...
class VentanaDescargas(gtk.Window):
    ...
    def set_layout(self):
        ...
        vbox1 = gtk.VBox()
        downloadstreaming = gtk.Button(_("Update JAMedia streams"))
        salir = gtk.Button(_("Close"))
    ...
    def get_JAMedia_streamings(self, widget):
        ...
        dialog = StandarDialog(
            self.ventanajamedia,
            _("JAMedia Message"),
            _("Download and update JAMedia stream?"))
    ...
    def download_streamings(self, widget=None, datos=None):
...
class ControlGrabar(gtk.Toolbar):
    ...
    def __init__(self, etiqueta):
        ...
        boton.show()
        tooltips = gtk.Tooltips()
        tooltips.set_tip(boton, _("Stop recording."), tip_private=None)
        self.show_all()
...
class ToolbarLista(gtk.Toolbar):
    ...
    __gsignals__ = {
    "load_list": (gobject.SIGNAL_RUN_FIRST,
        gobject.TYPE_NONE, (gobject.TYPE_INT,)),
    ...
    def __init__(self):
        ...
        imagen = gtk.Image()
        pixbuf = gtk.gdk.pixbuf_new_from_file_at_size(
            os.path.join(G.ICONOS, "lista.png"), 32, 32)
        imagen.set_from_pixbuf(pixbuf)
        boton.set_icon_widget(imagen)
        imagen.show()
        self.insert(boton, -1)
        boton.show()
        tooltips = gtk.Tooltips()
        tooltips.set_tip(boton, _("Select a list."), tip_private=None)
        ...
        imagen = gtk.Image()
        pixbuf = gtk.gdk.pixbuf_new_from_file_at_size(
            os.path.join(G.ICONOS, "agregar.png"), 32, 32)
        imagen.set_from_pixbuf(pixbuf)
    ...
    def get_menu(self, widget):
        ...
        item = gtk.MenuItem(_("JAMediaTube"))  # Quitar para Sugar
        menu.append(item)
        item.connect_object("activate", self.emit_load_list, 7)

        item = gtk.MenuItem(_("External Files"))
        menu.append(item)
